...
//diagonolize equation добавил fabs()????
 ...
 * I think we can remove fabs() here, because in that moment elements in A are positive
 * */
                ...
                const int &n)
{
    ...
    for (int i = 0; i < n; i++)
        fileOutput << std::setprecision(prec) << std::setw(width) << vector[i] << "\n";
    fileOutput << "\n";
